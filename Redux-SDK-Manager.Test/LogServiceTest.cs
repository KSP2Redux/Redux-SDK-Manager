...
public class LogServiceTest
{
    ...
    private const string LogsDir = @"C:\appdata\ReduxSdkManager\logs";
    ...
    public void Constructor_OpensNewLogFileMatchingPattern()
    {
        ...
        var files = fs.Directory.EnumerateFiles(LogsDir, "manager-*.log").ToList();
    ...
    [Test]
    public void Info_TagsEachLineWithProcessId()
    {
        var (fs, env) = BuildEnv();
        using var log = new LogService(fs, env);
    ...
    public void Constructor_WhenOverFileCap_DeletesOldestSoNewFilePlusNineRemain()
    {
        ...
        for (var i = 0; i < 10; i++)
        {
            var path = fs.Path.Combine(LogsDir, $"manager-2026010{i % 10}-000000.log");
        ...
        var remaining = fs.Directory.EnumerateFiles(LogsDir, "manager-*.log").ToList();
    ...
    public void Write_LogFileExceedsSizeCap_StopsWritingFurtherLinesToDisk()
    {
        ...
        // Push the file past the tiny test cap. Depending on the header length on the host, the cap
        // may already be exceeded here or only after this call, so a second call guarantees the
        // "reached cap" warning is written before the settled-size snapshot below.
        log.Info(new string('x', 250));
        log.Info("may or may not reach disk depending on header length");
